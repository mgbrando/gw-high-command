...
function MemberDetails(props) {
  if (props.loading && props.display) {
    ...
    const memberCharacters = props.characters.map(character => {
      return (
        <ListItem
          secondaryText={`${character.name}: Level ${character.level} ${
    ...
    /*const access;
    for(let content in props.accountInfo.access)
      content +=*/
    const access = props.accountInfo.access
      .reduce((fullAccess, singleAccess) => `${fullAccess}, ${singleAccess}`)
      .replace(/([A-Z, 0-9])/g, " $1")
      .trim();
    /*props.accountInfo.access.map(content =>
      content.replace(/([A-Z])/g, " $1").trim()
    );*/
    return (
      <div className="memberDetails">
        <Paper className="infoSection" zDepth={2}>
          <Paper className="memberDetailsInfo" zDepth={5}>
            ...
            <List className="membersDetailsList">
              <ListItem
                ...
                secondaryText={props.joined}
                secondaryTextLines={2}
                disabled={true}
              />
              <ListItem
                primaryText="Guilds "
                secondaryText={props.memberGuildNames}
                secondaryTextLines={2}
                disabled={true}
              />
            </List>
            <List className="membersDetailsList">
              <ListItem
                primaryText="Player Access "
                secondaryText={access}
                secondaryTextLines={2}
                disabled={true}
              />
              <ListItem
                primaryText="Commander "
                secondaryText={props.isCommander}
                secondaryTextLines={2}
                disabled={true}
              />
            </List>
